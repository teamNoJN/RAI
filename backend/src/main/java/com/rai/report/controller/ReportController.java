 ...
 *
 * CurrentUser 는 common 의 리졸버가 채운다 — Gateway 헤더가 있으면 그것을, 없으면 Bearer 토큰을
 * 검증하고, 둘 다 없으면 401. 회사 격리를 서비스 계층까지 내려보낸다.
 */
@RestController
...
public class ReportController {
    ...
    public ReportDto.CreateResponse create(CurrentUser currentUser,
                                           @Valid @RequestBody ReportDto.CreateRequest request) {
        return reportService.create(currentUser.companyId(), request);
    }

    /** 생성 폴링. job_id 는 POST 202 가 돌려준 값(=report_id)이다. */
    @GetMapping("/jobs/{job_id}")
    public ReportDto.JobResponse job(CurrentUser currentUser, @PathVariable("job_id") UUID jobId) {
        return reportService.getJob(currentUser.companyId(), jobId);
    }

    /** 5L 보관함 목록 (완료된 보고서만). */
    @GetMapping
    public List<ReportDto.ListItem> list(CurrentUser currentUser) {
        return reportService.list(currentUser.companyId());
    }

    /** 수정 채팅 — 호출 1건 = version +1. */
    @PatchMapping("/{report_id}")
    public ReportDto.ReviseResponse revise(CurrentUser currentUser,
                                           @PathVariable("report_id") UUID reportId,
                                           @Valid @RequestBody ReportDto.ReviseRequest request) {
        return reportService.revise(currentUser.companyId(), reportId, request.getInstruction());
    }

    /** PDF 내보내기 — 파일 스트림. */
    @GetMapping("/{report_id}/export")
    public ResponseEntity<byte[]> export(CurrentUser currentUser,
                                         @PathVariable("report_id") UUID reportId,
                                         @RequestParam(name = "format", defaultValue = "pdf") String format) {
        ReportPdf pdf = reportService.export(currentUser.companyId(), reportId, format);
